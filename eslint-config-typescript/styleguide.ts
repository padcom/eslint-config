...
async function lint(code: string) {
  ...
  const eslint = new ESLint({
    ...
    // @ts-ignore ignored, because it doesn't exist on the ESLint.Options
    ignore() {
...
/**
 * This is here to demonstrate things with variables
 */
function variables() {
...
// Use generics. They rock! Learn the advanced patterns and do use them!
 ...
 * This is how the most complex function looks like
 *
 * @param numberOfLoops number of loops to perform
 */
function whileLoops(numberOfLoops: number) {
  console.log('while loops', numberOfLoops)
...
class Person {
  ...
  // Tn TypeScript if you want you can also declare fields in constructor using scope modifiers.
  constructor(age: number, firstName: string, public lastName: string) {
...
console.log(new Person(42, 'Jane', 'Smith'))
...
async function arrowsCallbacks() {
  ...
  await Promise.resolve({ times: (x: number) => ({ shift: (y: number) => x * y }) }).then(x => x
    .times(2)
    // If you do that you will need to put the closing parent in the same line
    .shift(1))
}

// Either disregard the returned promise
void arrowsCallbacks()

// or handle any errors that might occur
arrowsCallbacks().catch(e => { console.error('Some error occurred', e) })
